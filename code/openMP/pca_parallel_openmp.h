...
void pca_parallel_openmp(int amountOfElements, int dimension, int dimensionToMapTo)
{
...
float **matrixMultiplication(float **matrixA, float **matrixB, int r1, int c1, int r2, int c2)
{
    ...
    #pragma omp parallel for schedule(dynamic, 1) shared(result) reduction(+:sum)
    for (int i = 0; i < r1; ++i)
    {
        for (int j = 0; j < c2; ++j)
        {
